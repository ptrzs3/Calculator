...
- 双向转换：数字量 → 模拟量 (D→A) / 模拟量 → 数字量 (A→D)
- 可配置分辨率：8 / 10 / 12 / 14 / 16 / 20 / 24 bit
- 可配置参考电压：Vref+ / Vref-，支持双极性（如 ±5V）
- 编码格式：Straight Binary / Two's Complement
- 常见 ADC 预设快速切换（9 组）
- 自动计算并显示 LSB 电压、±½ LSB 量化误差、最大编码值
